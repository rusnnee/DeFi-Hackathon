...
const INITIAL_EMPLOYEES: Employee[] = [
  { id: 'EMP001', name: 'Alice Chen', wallet_id: '0x6e48Ef77B14C0C81C5bf588Fc0e5937bf8aeCA46', salary: 5000, label: 'Engineer', chain: 'ETH', last_paid: '2026-01-28', status: 'ACTIVE' },
  { id: 'EMP002', name: 'Bob Martinez', wallet_id: '0x9f2aBC123def456ABC123def456ABC123def4567', salary: 4500, label: 'Designer', chain: 'BASE', last_paid: '2026-01-28', status: 'ACTIVE' },
  { id: 'EMP003', name: 'Carol Smith', wallet_id: '0x3d1DEF789abc012DEF789abc012DEF789abc0123', salary: 6000, label: 'PM', chain: 'ETH', last_paid: '2026-01-28', status: 'ACTIVE' },
  { id: 'EMP004', name: 'Dave Wilson', wallet_id: '0x7c4GHI345jkl678GHI345jkl678GHI345jkl6789', salary: 3500, label: 'DevOps', chain: 'ARB', last_paid: '2026-01-28', status: 'ACTIVE' },
  { id: 'EMP005', name: 'Eve Johnson', wallet_id: '0x1a8JKL901mno234JKL901mno234JKL901mno2345', salary: 5500, label: 'Data Analyst', chain: 'ETH', last_paid: 'never', status: 'PENDING' },
]
...
function EmployeeCard({ employee }: { employee: Employee }) {
  ...
  const truncatedWallet = wallet.length > 10
  ...
  const statusStyles = {
    ACTIVE: 'bg-[rgba(52,211,153,0.15)] text-usyc',
    PENDING: 'bg-[rgba(251,191,36,0.15)] text-amber',
    BLOCKED: 'bg-[rgba(248,113,113,0.15)] text-alert-red',
  }
  ...
  return (
    <div className="group rounded-[4px] border border-border bg-card p-4 transition-all hover:-translate-y-[2px] hover:shadow-[0_0_24px_rgba(192,132,252,0.2)]">
      {/* Top row: Avatar + Name/Role inline, status top-right */}
      <div className="flex items-start justify-between">
        ...
        <span className={`rounded-[3px] px-2 py-0.5 font-sans text-[11px] font-medium ${statusStyles[status]}`}>
          {status}
        </span>
      </div>
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        <button
          onClick={copyWallet}
          className="relative font-sans text-[11px] text-primary hover:text-secondary"
          aria-label="Copy wallet address"
        >
          {copied ? (
            <span className="text-usyc">Copied!</span>
          ) : (
      ...
      {/* Chain badge */}
      <div className="mt-2 flex items-center gap-2">
        <span className="rounded-[3px] bg-muted px-2 py-0.5 font-sans text-[11px] font-medium text-foreground">
          {employee.chain || 'ETH'}
        </span>
        <span className="font-sans text-[12px] text-muted-foreground">
          Last Paid: {employee.last_paid || 'Never'}
        </span>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(0,0,0,0.7)]" onClick={onClose}>
      <div className="relative w-full max-w-[480px] rounded-[6px] border border-border bg-card p-8" onClick={e => e.stopPropagation()}>
        {showFlyMoney && (
          <div className="fly-money absolute bottom-16 left-1/2 -translate-x-1/2">
            <FlyingMoneyIcon size={48} />
          </div>
        )}

        <div className="mb-6 flex items-center justify-between">
          <h2 className="font-sans text-[20px] font-bold text-foreground">Add Employee</h2>
          <button onClick={onClose} className="font-sans text-[18px] text-muted-foreground hover:text-foreground" aria-label="Close modal">
            X
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className={labelClass} htmlFor="emp-name">Name</label>
            <input id="emp-name" type="text" required value={name} onChange={e => setName(e.target.value)} className={inputClass} />
          </div>
          <div>
            ...
            <input
              ...
              onBlur={() => wallet && validateWallet(wallet)}
              className={`${inputClass} ${walletError ? 'border-alert-red' : ''}`}
              placeholder="0x..."
            />
            {walletError && <p className="mt-1 font-sans text-[12px] text-alert-red">{walletError}</p>}
          </div>
          <div>
            <label className={labelClass} htmlFor="emp-chain">Chain</label>
          ...
          <div>
            <label className={labelClass} htmlFor="emp-salary">Salary in USDC</label>
            <input id="emp-salary" type="number" required min="0" step="0.01" value={salary} onChange={e => setSalary(e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass} htmlFor="emp-currency">Currency</label>
          ...
          <div>
            <label className={labelClass} htmlFor="emp-label">Role / Label</label>
            <input id="emp-label" type="text" value={label} onChange={e => setLabel(e.target.value)} className={inputClass} />
          </div>
          <button
            type="submit"
            className="mt-2 w-full rounded-[4px] bg-gradient-to-r from-primary to-secondary px-5 py-[10px] font-sans text-[14px] font-medium text-primary-foreground transition-all hover:shadow-[0_0_12px_rgba(192,132,252,0.4)]"
          >
            Add to Payroll
          </button>
        </form>
...
function CSVUploadModal({ onClose, onAdd }: { onClose: () => void; onAdd: (emps: Employee[]) => void }) {
  ...
  function handleFile(file: File) {
    Papa.parse(file, {
      ...
      skipEmptyLines: true,
      complete(results) {
        setParsed(results.data as Record<string, string>[])
      },
      error() {
  ...
  function confirmUpload() {
    const newEmps: Employee[] = parsed.map((row, i) => ({
      id: `CSV${Date.now()}${i}`,
      name: row.name || 'Unknown',
      wallet_id: row.wallet || '',
      chain: row.chain || 'Ethereum',
      salary: parseFloat(row.amount) || 0,
        ...
        {parsed.length === 0 ? (
          <>
            <div
              ...
              className={`flex h-[160px] cursor-pointer flex-col items-center justify-center rounded-[4px] border-2 border-dashed transition-colors ${
                dragOver ? 'border-primary bg-[rgba(192,132,252,0.05)]' : 'border-border bg-input'
              }`}
              role="button"
              tabIndex={0}
            ...
            <p className="mt-3 font-sans text-[12px] text-muted-foreground">
              Required columns: name, wallet, chain, amount, currency, label
            </p>
          </>
        ) : (
          <>
            ...
            <button
              ...
              className="w-full rounded-[4px] bg-gradient-to-r from-primary to-secondary px-5 py-[10px] font-sans text-[14px] font-medium text-primary-foreground transition-all hover:shadow-[0_0_12px_rgba(192,132,252,0.4)]"
            >
              Confirm Upload
            </button>
          </>
...
export default function PayrollPage() {
  ...
  function handleCSVUpload(emps: Employee[]) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-[4px] bg-gradient-to-r from-primary to-secondary px-5 py-[10px] font-sans text-[14px] font-medium text-primary-foreground transition-all hover:shadow-[0_0_12px_rgba(192,132,252,0.4)]"
          >
            <PersonIcon size={16} />
            + Add Employee
          </button>
          <button
            ...
            className="flex items-center gap-2 rounded-[4px] border border-border bg-card px-5 py-[10px] font-sans text-[14px] font-medium text-foreground transition-colors hover:border-primary hover:text-primary"
          >
            <ScrollIcon size={16} />
            Upload CSV
          </button>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {sortedEmployees.length === 0 ? (
          <div className="col-span-full py-12 text-center">
            <p className="font-sans text-[14px] text-muted-foreground">No employees found. Add your first employee to get started.</p>
          </div>
        ) : (
      ...
      {/* Modals */}
      {showAddModal && (
        <AddEmployeeModal onClose={() => setShowAddModal(false)} onAdd={handleAddEmployee} />
      )}
      {showCSVModal && (
        <CSVUploadModal onClose={() => setShowCSVModal(false)} onAdd={handleCSVUpload} />
      )}
    </div>
  )
}
